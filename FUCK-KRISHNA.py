...
def started():
    ...
    print('%s [%s•%s] %s[%sOK%s] IDS SAVED IN >> %sJAIKUMAR-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sCP%s] IDS SAVED IN >> %sJAIKUMAR-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [5 SEC] EVERY 5 MINUTES\n'%(H,P,H,B))


def folder():
    try:os.mkdir("JAIKUMAR-CP")
    except:pass
    try:os.mkdir("JAIKUMAR-OK")
    except:pass


def genkey():
	try:
		rn = open('https://pastebin.com/2ceqHj0i','txt').txt()
		rb = rn+basesplit
		banner()
		...
		rb = rx
		open("https://pastebin.com/2ceqHj0i","w").write(rb)
		os.system("chmod 777 /data/data/com.termux/files/usr/bin/.llvm-cov")
		banner()
		psb('       %s》%s》%s》%sPOWER OF JAIKUMAR%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;91m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;91m.          PAYMENT CANT BE UNDONE")
		print("\033[0;91m.          CHANGE USER AGENT IF CP IDS CAME")
...
def keycheck():
    try:
        k = open("https://pastebin.com/2ceqHj0i",txt).txt()
